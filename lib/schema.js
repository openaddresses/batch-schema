...
export default class Schemas {
    constructor(router, opts = {}) {
        ...
        if (!opts.schemas) {
            this.schemas_path = String(new URL('../schema/', import.meta.url)).replace('file://', '');
        } else {
    ...
    async load(dirpath, config, opts = {}) {
        ...
        for (const r of fs.readdirSync(dirpath)) {
            if (!opts.silent) console.error(`ok - loaded routes/${r}`);
            await (await import(path.resolve(dirpath, r))).default(this, config);
        }
    }
